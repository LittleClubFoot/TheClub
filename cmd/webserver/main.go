...
import (
	...
	"github.com/LittleClubFoot/TheClub/internal/notes"
)

// templateFuncs provides custom functions available in all templates.
var templateFuncs = template.FuncMap{
	// safeHTML marks a string as safe rendered HTML (from goldmark markdown).
	"safeHTML": func(s string) template.HTML {
		return template.HTML(s)
	},
	// isImage returns true if the filename has an image extension.
	...
	// joinTags joins a string slice with ", " for display in form fields.
...
func main() {
	...
	// Determine notes data directory (configurable via env, defaults to ./data/notes)
	dataDir := os.Getenv("CLUB_NOTES_DIR")
	if dataDir == "" {
		dataDir = "data/notes"
	}

	store, err := notes.NewStore(dataDir)
	if err != nil {
		log.Fatalf("Failed to initialize notes store: %v", err)
	}

	authMW := auth.New()
	notesHandler := notes.NewHandler(store, authMW, templates)
	notesHandler.RegisterRoutes(mux)
	...
	fmt.Println("Starting The Club Go/HTMX Server")
	...
	fmt.Println("   /notes       - Notes dashboard (protected)")
	fmt.Println("   /notes/new   - Create new note (protected)")
	fmt.Println("   /notes/view/ - View note (public/protected)")
	fmt.Println("   /notes/qr/   - QR code for note")
	fmt.Printf("Notes data dir: %s\n", dataDir)
